...
// Pour parcourir les fichiers <------------------------ dans le fichier tag_hierarchy 
struct tag_t {
	char 			father[TAGSIZE];
	char 			name[TAGSIZE];
	int 			tag_id;
...
int add_tag(char *father, char *tag_name) {
	...
	int fdr = open("tag_list", O_RDONLY); //<--- le fichier doit exister ! Créé par Louis avec programme 0
	...
	int fdw = open("tag_list", O_WRONLY | O_APPEND);
	memset(buf, 0, TAGSIZE);
	buf[TAGSIZE - 1] = '%';
	...
	fdw = open("tag_hierarchy", O_WRONLY | O_APPEND);
...
void print_hierarchy() {
	...
	*id_count = 0;
	...
	int fdr = open("tag_hierarchy", O_RDONLY);
